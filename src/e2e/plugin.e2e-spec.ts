...
describe("example plugin", () => {
  ...
  describe("admin api", () => {
    it("adds an example", async () => {
      ...
      const {
        ...
        CreateExample.Mutation,
        CreateExample.Variables
      >(CREATE_EXAMPLE, {
        input: {
          name: initialName,
        },
    ...
    it("updates an example", async () => {
      const {
        ...
        UpdateExample.Mutation,
        UpdateExample.Variables
      >(UPDATE_EXAMPLE, {
        input: {
          id: exampleId,
          name: exampleName,
    ...
    it("returns a list of examples", async () => {
      ...
      } = await adminClient.query<GetExamples.Query, GetExamples.Variables>(
        GET_EXAMPLES
      );
    ...
    it("returns a single example", async () => {
      const { example } = await adminClient.query<
        GetExample.Query,
        GetExample.Variables
      >(GET_EXAMPLE, {
        id: "1",
      });
  ...
  describe("shop api", () => {
    it("returns a list of examples", async () => {
      const {
        examples: { items, totalItems },
      } = await shopClient.query<GetExamples.Query, GetExamples.Variables>(
        GET_EXAMPLES
      );
    ...
    it("returns a single example", async () => {
      const { example } = await shopClient.query<
        GetExample.Query,
        GetExample.Variables
      >(GET_EXAMPLE, {
        id: "1",
