...
export class ExampleDetailComponent
  extends BaseDetailComponent<Example>
  implements OnInit {
  ...
  constructor(
    ...
    this.detailForm = this.formBuilder.group({
      ...
      precio_dolar: ["", Validators.required],
    });
  }
  ...
  create(): void {
    ...
    const example: CreateExampleInput = {
      name: formValue.name,
      precio_dolar: formValue.precio_dolar,
    };
    this.dataService
      .mutate<CreateExample.Mutation, CreateExample.Variables>(CREATE_EXAMPLE, {
        input: example,
      })
      .subscribe(
        (data) => {
          ...
          this.router.navigate(["../", data.createExample.id], {
  ...
  private saveChanges(): Observable<boolean> {
    if (this.detailForm.dirty) {
      ...
      return this.dataService
        .mutate<UpdateExample.Mutation, UpdateExample.Variables>(
          UPDATE_EXAMPLE,
          {
            input,
          }
  ...
  protected setFormValues(entity: Example): void {
    this.detailForm.patchValue({
      name: entity.name,
      precio_dolar: entity.precio_dolar,
    });
  }
}
